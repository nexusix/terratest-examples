...
func TestPackerBasicExample(t *testing.T) {
	...
	defer terratestAWS.DeleteAmiAndAllSnapshots(t, awsRegion, amiID)

	// Check if AMI is shared/not shared with account
	requestingAccount := terratestAWS.CanonicalAccountId
	randomAccount := "123456789012" // Random Account
	ec2Client := terratestAWS.NewEc2Client(t, awsRegion)
	ShareAmi(t, amiID, requestingAccount, ec2Client)
	accountsWithLaunchPermissions := terratestAWS.GetAccountsWithLaunchPermissionsForAmi(t, awsRegion, amiID)
	...
	amiIsPublic := terratestAWS.GetAmiPubliclyAccessible(t, awsRegion, amiID)
...
func TestPackerMultipleConcurrentAmis(t *testing.T) {
	...
	for key, amiId := range resultMap {
		...
		terratestAWS.DeleteAmiAndAllSnapshots(t, awsRegion, amiId)
	}
}
